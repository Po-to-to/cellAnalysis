...
class Image:
    ...
    def __init__(self, file_path, channel_names: list[str] = None):
        # or __post_init__
    ...
    def load_image(self):
        ...
        """loads the image data and stores it in self.image"""
        if self.image_path.suffix == '.tif':
            self.image = io.imread(self.image_path)
        elif self.image_path.suffix == '.czi':
            self.image = AICSImage(self.image_path)
            self.image = self.image.asarray()
        else:
            raise ValueError(f"Extention '{self.image_path.suffix}' is not supported!")
